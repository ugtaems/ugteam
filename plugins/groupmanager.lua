...
local lang = redis:get(hash)
    -- superuser and admins only (because sudo are always has privilege)
      if not is_admin(msg) then
     if not lang then
        return '⛔️*Yσυ αяє ησт вσт α∂мιη*🚷'
   else
        return '⛔️_شما مدیر ربات نیستید_🚷'
    end
   end
    local data = load_data(_config.moderation.data)
...
local lang = redis:get(hash)
  ...
  if not lang then
    return "⛔️_Nσ_ *мσ∂єяαтσя* _ιη тнιѕ gяσυρ_❗️❕"
else
   return "⛔️_در این گروه هیچ معاونی برای ربات انتخاب نشده است❗️👤_"
  end
end
if not lang then
...
if not tonumber(data.sender_user_id_) then return false end
  ...
  if not lang then
    return tdcli.sendMessage(data.chat_id_, "", 0, "⛔️*Gяσυρ ιѕ ησт α∂∂є∂❌❗️*", 0, "md")
else
    return tdcli.sendMessage(data.chat_id_, "", 0, "_⛔️گروه درلیست گروه های پشتیبانی نیست❌❗️_", 0, "md")
     end
  end
...
administration[tostring(arg.chat_id)]['owners'][tostring(data.id_)] = user_name
   ...
   if not lang then
  return tdcli.sendMessage(arg.chat_id, "", 0, "👑️👤_Uѕєя_ "..user_name.." *"..data.id_.."* _ιѕ ησω тнє_ *gяσυρ σωηєя*✅", 0, "md")
   else
...
elseif lang then
 return "🔰ارسال کلمه فارسی عربی در گروه ممنوع است🔒"
end
else
...
if not lang then
 ...
 return "🔰ارسال کلمه فارسی وعربی در گروه ممنوع شد🔒"
end
end
end
...
elseif lang then
return "🔰 ارسال کلمه  فارسی و عربی در گروه ممنوع نیست🔓"
end
else
...
return "🔰 ارسال  کلمه فارسی و عربی در گروه ازاد شد🔓"
end
end
end
...
elseif lang then
return "🔰 بیصدا کردن ارسال مخاطب فعال نیست🔊"
end
else
...
local settings = data[tostring(target)]["settings"]
 text = "**\n*تنظیمات گروه:*\n*___________________________*\n لیست قفل ها :\n_🔒>>قفل_ ویرایش پیام : *"..settings.lock_edit.."*\n_🔒>> قفل لینک: *"..settings.lock_link.."*\n_🔒>> قفل_ تگ : *"..settings.lock_tag.."*\n_🔒>>قفل_پیام مکرر : *"..settings.flood.."*\n_🔒>>قفل_فحش : *"..settings.fosh.."*\n_🔒>>قفل_هرزنامه : *"..settings.lock_spam.."*\n_🔒>>قفل_فراخوانی : *"..settings.lock_mention.."*\n_🔒>>قفل_فارسی : *"..settings.lock_arabic.."*\n_🔒>>قفل_ انگلیسی : *"..settings.english.."*\n_🔒>>قفل_تبچی : *"..settings.tab.."*\n_🔒>>قفل_تبلیغات : *"..settings.ads.."*\n_🔒>>قفل_صفحات وب : *"..settings.lock_webpage.."*\n_🔒>>قفل_فونت : *"..settings.lock_markdown.."*\n_🔒>>قفل_ سنجاق کردن : *"..settings.lock_pin.."*\n*___________________________*\n_➕پیام خوشآمد گویی :_ *"..settings.welcome.."*\n_➕محافظت در برابر ربات ها :_ *"..settings.lock_bots.."*\n_➕حداکثر پیام مکرر :_ *"..NUM_MSG_MAX.."*\n"..expirefa.."\n*___________________________*\n *لیست ممنوعیت ها*:\n_🔇>>بیصدا_همه:*"..settings.mute_all.."*\n_🔇>>بیصدا_تصاویرمتحرک: *"..settings.mute_gif.."*\n_🔇>>بیصدا_متن :*"..settings.mute_text.."*\n🔇>>بیصدا_کیبورد شیشه ای: *"..settings.mute_inline.."*\n_🔇بیصدا_ بازی تحت  وب : *"..settings.mute_game.."*\n_🔇>>بیصدا_عکس: *"..settings.mute_photo.."*\n_🔇>>بیصدا_فیلم: *"..settings.mute_video.."*\n_🔇>>بیصدا_آهنگ: *"..settings.mute_audio.."*\n_🔇>>بیصدا_ویس: *"..settings.mute_voice.."*\n_🔇>>بیصدا_برچسب: *"..settings.mute_sticker.."*\n_🔇>>بیصدا_ ارسال مخاطب : *"..settings.mute_contact.."*\n_🔇>>بیصدا_ نقل قول : *"..settings.mute_forward.."*\n_🔇>>بیصدا_موقعیت : *"..settings.mute_location.."*\n_🔇>>بیصدا_اسناد : *"..settings.mute_document.."*\n_🔇>>بیصدا_ خدمات تلگرام : *"..settings.mute_tgservice.."*\n_🔇>>بیصدا_صفحه کلید : *"..settings.mute_keyboard.."*\n*___________________________*\nکانال ما: @ugtaim \n ساخته شده توسط :🌷🌺ℳʊⓢ@🌺🌷 \n_زبان سوپرگروه_ : *fa*"
end
text = string.gsub(text, "yes", "уєѕ")
text = string.gsub(text, "no", "no")
return text
end
    ...
    if matches[1]:lower() == 'پاک کردن' and is_owner(msg) then
      if matches[2] == ' لیست معاون ها' then
        ...
        return "_All_ *moderators* _has been demoted_"
            ...
            return "تمام معاون ها از گروه عزل شدند"
      end
         end
...
text = [[
🔰راهنمای دستورات ربات ضد لینک ug🔰
➖➖➖➖➖➖➖➖➖➖
📝 دستورات قفل کردن و بازکردن قفل ها
🔒 قفلی 
➖➖➖➖➖➖➖➖➖➖
📝 دستورات ممنوعیتهای ربات  ضد لینک
🔒 ممنوعیت
➖➖➖➖➖➖➖➖➖➖
📝 دستورات برای مدیریت گروه
🛂  مدیریتی
➖➖➖➖➖➖➖➖➖➖
📝 دستورات تعطیل کردن گروه 
🛂 راهنماتعطیلی
➖➖➖➖➖➖➖➖➖➖
📝 دستورات عمومی و سرگرم کننده در گروه
🎉 فان 
➖➖➖➖➖➖➖➖➖➖
💢 مدیران و ادمین های عزیز میتوان با زدن دستورات بالا گروه خودرا مدیریت کنن 💢
➖➖➖➖➖➖➖➖➖➖
💢 ربات ضد لینک ug دوزبانه بوده و با زدن دستور زیر میتوانید زبان ربات را تنظیم کنید 💢
📝 تنظیم زبان 
✏️ مثال:تنظیم زبان فارسی
➖➖➖➖➖➖➖➖➖➖
📝 دستور نرخ جهت خرید ربات ضد لینک 
💵 نرخ
📝 اطمینان از انلاین بودن ربات
👀 انلاینی
📝 نمایش دستورات صاحب ربات این دستور برای ادمین ها ومدیران نمی باشد
⛔️ سودو 
➖➖➖➖➖➖➖➖➖➖
📝دیدن فعال /غیرفعال بودت دستورات 
🔹 تنظیمات
➖➖➖➖➖➖➖➖➖➖
...
if matches[1] == "قفلی" and is_mod(msg) then
text = [[🔰راهنمای دستورات قفل های ربات ضد لینک ug🔰
➖➖➖➖➖➖➖➖➖➖
💬 قفل کردن لینک 
🔒 قفل لینک
🔓 بازکردن لینک
...
text = [[
🔰راهنمای دستورات ممنوعیت های ربات ضد لینک ug🔰
➖➖➖➖➖➖➖➖➖➖
💬 ممنوع کردن همه 
🔇 بیصدا همه
🔊 باصدا همه
...
🔊 باصدا فوروارد
➖➖➖➖➖➖➖➖➖➖
💬 ممنوع کردن ارسال مکان
🔇 بیصدا مکان
🔊 باصدا مکان
➖➖➖➖➖➖➖➖➖➖
💬 ممنوع کردن ارسال فایل
🔇 بیصدا فایل
🔊 باصدا فایل
➖➖➖➖➖➖➖➖➖➖
💬 ممنوع کردن خدمات تلگرام
🔇 بیصدا خدمات تلگرام
🔊 باصدا خدمات تلگرام
➖➖➖➖➖➖➖➖➖➖
💬 ممنوع کردن دکمه شیشه ای
🔇 بیصدا دکمه شیشه ای
🔊 باصدا دکمه شیششه ای
➖➖➖➖➖➖➖➖➖➖
💬 ممنوع کردن بازی تحت وب
🔇 بیصدا بازی
🔊 باصدا بازی
➖➖➖➖➖➖➖➖➖➖
💬 ممنوع کردن کیبورد
...
🔹 ازاد
➖➖➖➖➖➖➖➖➖➖
📝 نمایش شناسه وسمت  کاربر در گروه
🔹 اطلاعات یوزرنیم 
🔹 ایدی
🔹 من
🔹 مقام من
...
🔹 اطلاعات ایدی(ایدی)
➖➖➖➖➖➖➖➖➖➖
📝 دستور اخطار دادن به کاربر خاطی
🔹 اخطار 		
📝 برای تنظیم کردن حداکثر اخطار در گروه
🔹حداکثر اخطار
📝برای حذف  اخطار  کاربر اخطار گرفته 
🔹حذف اخطار
مثال:حداکثر اخطار5		
 ➖➖➖➖➖➖➖➖➖➖	
...
🔹 لینک پیوی
➖➖➖➖➖➖➖➖➖➖
📝 پاکسازی تعداد پیام اخیر سوپر گروها 
🔹 پاکسازی  
🔹 پاکسازی همه   
🔹 پاکسازی پیام من  
📝 پاکسازی همه کل پیام های گروه رو پاک میکنه  
📝 پاکسازی پیام من  فقط پیام شمارو پاک میکنه ازگروه 
🔹 مثال:پاکسازی + عدد
